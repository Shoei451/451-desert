/**
 * Card Generator for Solarized Template
 * Usage: generateCards(items, containerId)
 */
...
function generateCards(items, containerId) {
    ...
    items.forEach((item, index) => {
        ...
        cardElement.innerHTML = `
            ...
            ${item.titleEN ? `<p class="subtitle">${item.titleEN}</p>` : ''}
            <p>${item.description}</p>
        `;
        
        container.appendChild(cardElement);
        
        // Trigger animation immediately after adding to DOM
        setTimeout(() => {
...
// Initialize animation observer for dynamically loaded cards
